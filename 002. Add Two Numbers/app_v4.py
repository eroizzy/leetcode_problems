...
class Solution:
    ...
    def addTwoListNodesRecursive(
        self, l1: Optional[ListNode], l2: Optional[ListNode], remainder: Optional[int]
    ) -> Optional[ListNode]:
        if l1 != None and l2 != None:
            add: int = l1.val + l2.val + remainder
            return ListNode(
                add % 10, self.addTwoListNodesRecursive(l1.next, l2.next, int(add / 10))
            )
        elif l1 != None and l2 == None:
            add: int = l1.val + remainder
            return ListNode(
                add % 10, self.addTwoListNodesRecursive(l1.next, l2, int(add / 10))
            )
        elif l1 == None and l2 != None:
            add: int = l2.val + remainder
            return ListNode(
                add % 10, self.addTwoListNodesRecursive(l1, l2.next, int(add / 10))
            )
        else:
            if remainder != 0:
